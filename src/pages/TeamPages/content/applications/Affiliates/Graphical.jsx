...
const Graphical = ({ dateFilters, setDateFilters, paginatedCryptoOrders }) => {
  ...
  const [percentData, setPercentData] = useState([0, 0]);
  ...
  useEffect(() => {
    ...
    paginatedCryptoOrders?.forEach((item) => {
      const affiliate_data = item?.affiliate_data?.paid_status;
      if (affiliate_data === "Paid") {
        paidCount++;
      } else if(affiliate_data === "Unpaid") {
        unPaidCount++;
      }
    });
    ...
    const percentages = [paidCount, unPaidCount];
  ...
  return (
    <div className="flex md:flex-row flex-col items-center md:items-center  justify-start gap-10 md:gap-2 py-5">
      {" "}
      <div>
