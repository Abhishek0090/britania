...
const MarksChart = ({
  ...
  // // You can add this property to totalData if needed
  // totalData?.total_marks_out_of_100 = total_marks_out_of_100;

  // TODO: Fix the crash issue over here
  ...
  useEffect(() => {
    const percentDetails = paginatedCryptoOrders.reduce(
      (acc, item) => {
        acc[0] += parseInt(item?.freelancer_resit);
        acc[1] += parseInt(item?.freelancer_passing);
        acc[2] += parseInt(item?.freelancer_merit);
        acc[3] += parseInt(item?.freelancer_distinction);
  ...
  return (
    <div className="flex md:flex-row flex-col items-center md:items-center  justify-between px-0 md:px-20 gap-10 md:gap-2 py-5">
      {" "}
      <div>
      ...
      {(dateFilters?.months || dateFilters?.category || dateFilters?.years) && (
        <div>
