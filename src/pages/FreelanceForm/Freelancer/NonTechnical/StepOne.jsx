...
export default function SetpOne() {
  ...
  let freelancerData = useSelector(
  ...
  const [role, setRole] = useState("Non Technical");
      ...
      .then((res) => {
        ...
        dispatch(
          setNonTechnicalFreelancerFormData({
            ...
            past_work_files_random_number: res?.data?.random_number,
          })
        );
        // console.log(res);
      })
  ...
  const handleDeleteFile = (file) => {
    ...
    if (index === -1) {
      let previousFiles = freelancerData?.past_work_files
        ?.split("_$_")
        ?.filter((item) => item !== "")
