...
const CompletedMessages = ({
  ...
  const handleClick = () => {
    ...
    const membersIdss = totalTeamNames.map((teamMember) => ({
      id: teamMember.id,
      name: teamMember.name,
    }));

    const filteredMapIds = membersIdss?.filter((teamMember) => teamMember.id !== auth?.id);
    const loggedInUserId = membersIdss?.filter((teamMember) => teamMember.id === auth?.id);
